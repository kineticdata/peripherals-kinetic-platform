...
class KineticRequestCeNotificationTemplateBuildV1
  ...
  def execute
    ...
    # Return Results
	  ...
	  results += "  <result name='To'>#{escape(@recipient_json["smtpaddress"])}</result>\n"
	  results += "  <result name='Subject'>#{escape(template_to_use["Subject"])}</result>\n"
      results += "  <result name='HTML Body'>#{escape(template_to_use["HTML Content"])}</result>\n"
	  results += "  <result name='Text Body'>#{escape(template_to_use["Text Content"])}</result>\n"
      results += "</results>"
    end
    return results
  end
