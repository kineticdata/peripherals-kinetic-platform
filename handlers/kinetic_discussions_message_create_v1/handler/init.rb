...
class KineticDiscussionsMessageCreateV1
  ...
  begin
    ...
    data = %|{"content": #{@parameters['message']}}|

    auth_token = get_auth_token
    resource = RestClient::Resource.new("#{@discussions_api_url}/v1/discussions/#{@parameters['guid']}/messages")
    result = resource.post(data, { accept: :json, content_type: :json, authorization: "Bearer #{auth_token}" })
    ...
    <results>
        ...
        <result name="Message">#{result.nil? ? '' : escape(result.body)}</result>
    </results>
    RESULTS
  end
