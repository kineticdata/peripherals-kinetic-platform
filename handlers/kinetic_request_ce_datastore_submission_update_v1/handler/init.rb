...
class KineticRequestCeDatastoreSubmissionUpdateV1
  ...
  def execute
    ...
    begin
      api_route = "#{server}/app/api/v1/datastore/submissions/#{submission_id}"
      ...
      data.tap do |json|
        json[:currentPage] = {
                               "name" => (@parameters["current_page_name"] if !@parameters["current_page_name"].to_s.empty?),
                               "navigation" => (@parameters["current_page_navigation"] if !@parameters["current_page_navigation"].to_s.empty?)
                             }
