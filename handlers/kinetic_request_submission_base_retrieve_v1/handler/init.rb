...
class KineticRequestSubmissionBaseRetrieveV1
  ...
  def execute()
    ...
    entry = @@remedy_forms['KS_SRV_CustomerSurvey_base'].find_entries(
      :single,
      :conditions => [%|'CustomerSurveyID' = "#{@parameters['Submission ID']}" or 'instanceId' = "#{@parameters['Submission ID']}"|],
      :fields => :all
    )

    if entry.nil?
      raise(%|An entry can't be found in KS_SRV_CustomerSurvey_base for (Request ID or instanceId)="#{@parameters['Submission ID']} "|)
    end
    ...
    if @debug_logging_enabled then
      field_values = entry.field_values.collect do |field_id, value|
        "#{@@remedy_forms['KS_SRV_CustomerSurvey_base'].field_for(field_id).name}: #{value}"
      end
	    puts("Field Values: #{field_values.inspect}")
    end
    
    base_results = ""    
    entry.field_values(:name).each_pair {|key, value|
      base_results << %|<result name="#{escape(key)}">#{escape(value)}</result>|
    }
