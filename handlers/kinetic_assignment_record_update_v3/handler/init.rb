...
class KineticAssignmentRecordUpdateV3
  ...
  def execute()
    ...
    if entry.nil?
      raise(%|An entry can't be found in KS_RQT_Assignment for (Work Order ID)="#{@parameters['Work Order ID']}"|)
    end

	entry.update_attributes!(@field_values)
