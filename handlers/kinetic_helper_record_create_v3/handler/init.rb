...
class KineticHelperRecordCreateV3
  ...
  def initialize(input)
    ...
    REXML::XPath.match(@input_document, '/handler/fields/field').each do |node|
		if (node.text != "" && !node.text.nil?)
			if (node.attribute('name').value == "Character Field1" || node.attribute('name').value == "Character Field2" || node.attribute('name').value == "Character Field3" || node.attribute('name').value == "Character Field4" || node.attribute('name').value == "Character Field5" || node.attribute('name').value == "Character Field6" || node.attribute('name').value == "Character Field7" || node.attribute('name').value == "Character Field8" || node.attribute('name').value == "Character Field9" || node.attribute('name').value == "Character Field10" || node.attribute('name').value == "Character Field11" || node.attribute('name').value == "Character Field12")
			@field_values[node.attribute('name').value] = node.text[0..254]
			else
  ...
  def execute()
    ...
    helper_entry = @@remedy_forms['KS_SRV_Helper'].find_entries(
      :single,
      :conditions => [%|'SurveyInstanceID' = "#{@field_values['SurveyInstanceID']}" AND 'CustomerSurveyInstanceID' = "#{@field_values['CustomerSurveyInstanceID']}" AND 'Status' = "Active"|],
      :fields => []
    )
    if !helper_entry.nil?
      raise(%|An active entry already exists in KS_SRV_Helper for (SurveyInstanceID)="#{@field_values['SurveyInstanceID']} AND (CustomerSurveyInstanceID)="#{@field_values['CustomerSurveyInstanceID']} "|)
    end
