...
class KineticDiscussionsDiscussionUninviteV1
  ...
  begin
    ...
    if @parameters['invited_user'].to_s.strip.empty? && @parameters['invited_email'].to_s.strip.empty? then
      raise "Either 'Invited CE Username' or 'Invited Email Address' parameters must be supplied."
    end
    
    invitation_url = "#{@discussions_api_url}/v1/discussions/#{@parameters['guid']}/invitations/"
    if @parameters['invited_email'].to_s.strip.empty? == false then
      invitation_url << "#{URI.encode(@parameters['invited_email'])}?email=true"
    else
      invitation_url << "#{URI.encode(@parameters['invited_user'])}"
    end
	
    auth_token = get_auth_token
    resource = RestClient::Resource.new(invitation_url)
    result = resource.delete({ accept: :json, content_type: :json, authorization: "Bearer #{auth_token}" })
