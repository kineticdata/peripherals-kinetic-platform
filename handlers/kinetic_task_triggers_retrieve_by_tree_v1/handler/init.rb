...
class KineticTaskTriggersRetrieveByTreeV1
  def initialize(input)
    ...
    @tree_id = get_parameter_value(@input_document, 'tree_id')
  end

  def execute()
    # This is the qualification that is used to retrieve paused triggers that
    # were created due to a paused tree being processed.
    qualification = [
      %|'flags' = "PausedTree"|,
      %|'Status' = "Paused"|,
      %|'task_tree_instanceId' = "#{@tree_id}"|
      ].join(" AND ")
    ...
    # Begin building the XML output by iterating through each of the returned
    # trigger records.
    xml = '<triggers>'
