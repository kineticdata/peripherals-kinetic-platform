...
class KineticRequestSubmissionCloseAllOpenV1
  ...
  # Updates the value of the 'Request_Status' and 'ValidationStatus' fields on
  # the specified Kinetic Request base record.
  #
  # This is a required method that is automatically called by the Kinetic Task
  # Engine.
  #
  ...
  def execute()
	...
	query = '\'OriginatingID\' = "' +@parameters['submission_id'] + '" AND \'Request_Status\' = "Open"'
	# Check for existing KS_SRV_Helper entry.
    entries = @@remedy_forms['KS_SRV_CustomerSurvey_base'].find_entries(
      :all,
      :conditions => [query],
      :fields => [179]
    )
    if entries.nil?
      raise(%|An open entry can't be found in KS_SRV_CustomerSurvey_base for (Query)="#{query} "|)
    end
