...
class KineticHelperRecordUpdateV2
  ...
  def execute()
    ...
    # Update the KS_SRV_Helper record using the @field_values hash
    # that was built up.
	
	@@remedy_forms['KS_SRV_Helper'].update_entry!(
	helper_entry, :field_values => @field_values
    )

    # Build the results xml that will be returned by this handler.
    results = <<-RESULTS
    <results>
      <result name="Entry Id">#{escape(helper_entry.id)}</result>
    </results>
    RESULTS
