...
class KineticRequestServiceItemDetectAttributeV1
  ...
  def execute()
    ...
    if service_item_entry.nil?
      raise(%|Could not find service item named "#{@parameters['service_item_name']}" in the catalog "#{@parameters['catalog_name']}"|)
    end

    # Query the attribute instance form with the specified attribute name and the
    # instanceId of the service item record.  The result will be used to build
    # the Yes/No string returned by this handler.
    ...
    result = attribute_entry.empty? ? "No" : "Yes"

    # Build and return the results string returned by this handler.
    <<-RESULTS
    <results>
      <result name="Exists">#{escape(result)}</result>
    </results>
    RESULTS
  end
