...
class KineticRequestSubmissionUpdateAnswersV5
  ...
  def execute()
	...
	puts("Found Submission") if @debug_logging_enabled
	...
	puts("Combined Answer Set: \n#{combined_answer_set}") if @debug_logging_enabled
    ...
    questions = @@remedy_forms['KS_SRV_SurveyQuestion'].find_entries(
      :all,
      :fields => ['Editor Label', 'instanceId', 'FieldMapNumber', 'Answer Mapping', 'Answer_MappingFieldID'],
      :conditions => [%|'SurveyInstanceID'="#{submission['SurveyInstanceID']}"|]
    )
    questions.each do |question|
	  #puts("Processing question: \n#{question['Editor Label']}") if @debug_logging_enabled
      if combined_answer_set.has_key?(question['Editor Label'])
        ...
        unless (answer.nil? || (answer.is_a?(String) && answer.empty?))
			currentAnswer = @@remedy_forms['KS_SRV_SurveyAnswer'].find_entries(
			  :single,
			  :conditions => [%|'CustomerSurveyInstanceID'="#{submission['instanceId']}" AND 'QuestionInstanceID'="#{question['instanceId']}"|],
			  :fields       => ['instanceId']
			)
			if currentAnswer.nil?
				puts("Creating Answer for #{question['Editor Label']}: \n#{answer}") if @debug_logging_enabled
				...
				puts("Updating Answer for #{question['Editor Label']}: \n#{answer}") if @debug_logging_enabled
				if answer.bytesize > 4000
					currentAnswer.update_attributes!({
						'AnswerViewer'              => answer[0,254],
						'FullAnswer'                => answer[0,3999],
						'UnlimitedAnswer'           => answer
					})
			...
			if question['Answer_MappingFieldID'] != nil
			
			  field_id = question['Answer_MappingFieldID']
			...
			# Update the Base Entry's mapped Attribute.
			  submission.update_attributes!(field_name => answer[0,254])
