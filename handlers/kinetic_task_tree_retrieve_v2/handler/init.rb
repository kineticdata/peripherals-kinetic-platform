...
class KineticTaskTreeRetrieveV2
  ...
  def execute()
    ...
    title = @parameters["source"] + " :: " + @parameters["group"] + " :: " + @parameters["tree"]
    puts "Tree Title: #{title}" if @enable_debug_logging
    ...
    # API Route with Includes
      task_api_route = server + "/app/api/v2/trees/" + URI.encode(title) + "?include=details,treeXml"
