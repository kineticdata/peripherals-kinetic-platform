...
class KineticCoreBridgeCallV1
  ...
  def initialize(input)
    ...
    @path += "/#{@kapp}/#{@form}/bridgedResource/#{@resource}" if !@path.start_with?("/")

    #process given Parameters
    if !@parameters["parameters"].nil?
      @pathAddition = "?"
      @values = JSON.parse(@parameters["parameters"])
      @values.each do |key,value|
        @pathAddtion += CGI.escape(key) + "=" + CGI.escape(value)
      end
    @path += @pathAddition
    end
