...
class KineticRequestCeSubmissionSearchV2
  ...
  def execute
    ...
    begin
      ...
      api_route += "&end=#{URI.encode(@parameters['end'])}" if !@parameters['end'].to_s.empty?
      api_route += "&limit=#{@parameters['limit']}" if !@parameters['limit'].to_s.empty?
      api_route += "&pageToken=#{@parameters['page_token']}" if !@parameters['page_token'].to_s.empty?
