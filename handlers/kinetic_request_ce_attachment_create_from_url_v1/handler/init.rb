...
class KineticRequestCeAttachmentCreateFromUrlV1
  ...
  def execute
    ...
    begin
      ...
      # Fetch File from Url
      file_dl_response = RestClient.get(url)
      ...
      byte = ByteArrayBody.new(file_dl_response.body.to_java_bytes,file_dl_response.headers[:content_type], @parameters["filename"])
    ...
    <results>
      ...
      <result name="Handler Error Message">#{error_message.nil? ? "" : escape(error_message)}</result>
    </results>
    RESULTS
  end
